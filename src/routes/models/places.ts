...
router.get(
	'/',
	modelQueryMiddleWare('Places', true),
...
router.get(
	'/one',
	modelQueryMiddleWare('Places', false),
	async (req: Request, res) => {
		const client: CosmosClient = req.app.get('client');
		const { resources } = await query(client, 'Places', {
			query: `SELECT * FROM r ${req.querySpec?.query}`,
 ...
 * POST
 */
router.post('/', modelBodyMiddleware('Places'), async (req: Request, res) => {
	const client: CosmosClient = req.app.get('client');
 ...
 * PUT
 */
router.put('/:id', modelBodyMiddleware('Places'), async (req: Request, res) => {
	if (
